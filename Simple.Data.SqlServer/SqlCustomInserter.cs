...
namespace Simple.Data.SqlServer
{
    ...
    {
        #region ICustomInserter Members
                                                  ...
                                                  IDbTransaction transaction = null, bool resultRequired = false)
        {
            ...
            Dictionary<Column, object> dataDictionary = BuildDataDictionary(adapter, data, table);
            ...
            if (resultRequired)
            {
                Column identityColumn = table.Columns.FirstOrDefault(c => c.IsIdentity);
        ...
        private static Dictionary<Column, object> BuildDataDictionary(AdoAdapter adapter,
                                                                      IEnumerable<KeyValuePair<string, object>> data, Table table)
        {
            ...
            Dictionary<Column, object> dataDictionary = data.Where(kvp => columnFilter(kvp.Key))
                .ToDictionary(kvp => table.FindColumn(kvp.Key), kvp => kvp.Value);
            return dataDictionary;
        }

        internal IDictionary<string, object> ExecuteSingletonQuery(AdoAdapter adapter, string sql,
                                                                   ...
                                                                   IDbTransaction transaction)
        {
            if (transaction != null)
            {
                IDbCommand command = new CommandHelper(adapter).CreateInsert(transaction.Connection, sql, columns,
                                                                             values.ToArray());
            ...
            {
                using (
                    IDbCommand command = new CommandHelper(adapter).CreateInsert(connection, sql, columns,
                                                                                 values.ToArray()))
                {
        ...
        internal int Execute(AdoAdapter adapter, string sql, IEnumerable<Column> columns, IEnumerable<Object> values,
                             IDbTransaction transaction)
        {
            if (transaction != null)
            {
