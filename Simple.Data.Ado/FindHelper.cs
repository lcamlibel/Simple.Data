...
namespace Simple.Data.Ado
{
    internal class FindHelper
    {
        ...
        public ICommandBuilder GetFindByCommand(ObjectName tableName, SimpleExpression criteria)
        {
            ...
            if (criteria != null)
            {
                ...
                _commandBuilder.Append(string.Join(" ",
                                                   new Joiner(JoinType.Inner, _schema).GetJoinClauses(tableName,
                                                                                                      criteria)));
        ...
        private string GetSelectClause(ObjectName tableName)
        {
            ...
            return string.Format("select {0} from {1}", string.Join(", ", table.Columns.Select(c => c.QualifiedName)),
                                 table.QualifiedName);
        }
    }
    ...
    internal class GetHelper
    {
        ...
        public ICommandBuilder GetCommand(Table table, params object[] keyValues)
        {
            ...
            ParameterTemplate param = _commandBuilder.AddParameter(keyValues[0], table.FindColumn(table.PrimaryKey[0]));
            _commandBuilder.Append(string.Format(" where {0} = {1}", _schema.QuoteObjectName(table.PrimaryKey[0]),
                                                 param.Name));
            for (int i = 1; i < table.PrimaryKey.Length; i++)
            {
                ...
                _commandBuilder.Append(string.Format(" and {0} = {1}", _schema.QuoteObjectName(table.PrimaryKey[i]),
                                                     param.Name));
            }
